...
export default function UploadPage() {
  ...
  const handleVideoSelect = (e) => {
    ...
    if (file.size > 100 * 1024 * 1024) { toast.error('Video size must be less than 100MB'); return; }
    setVideoFile(file);
  ...
  const handleReset = () => {
    ...
    setVideoPreview(null); setThumbnailPreview(null);
    setCaption(''); setErrors({});
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              {!videoPreview ? (
                <div onClick={() => videoInputRef.current?.click()}
                  className={`border-2 border-dashed rounded-2xl p-12 text-center cursor-pointer hover:border-red-400 transition-colors ${errors.video ? 'border-red-500' : 'border-gray-200'}`}>
              ...
              <div className="flex items-center gap-4">
                {thumbnailPreview ? (
                  ...
                  <div onClick={() => thumbnailInputRef.current?.click()}
                    className="w-24 h-36 border-2 border-dashed border-gray-200 rounded-xl flex items-center justify-center cursor-pointer hover:border-red-400">
            ...
            <div>
              ...
              <textarea value={caption}
                onChange={(e) => { setCaption(e.target.value); if (errors.caption) setErrors((prev) => ({ ...prev, caption: '' })); }}
                placeholder="Write a caption for your video..."
                maxLength={150} rows={3}
                className={`w-full px-4 py-3 border rounded-xl resize-none focus:outline-none focus:ring-2 focus:ring-red-400 ${errors.caption ? 'border-red-500' : 'border-gray-200'}`} />
              <div className="flex justify-between items-center mt-1">
                {errors.caption ? <p className="text-red-500 text-xs">{errors.caption}</p> : <span />}
                <p className={`text-xs ${caption.length > 130 ? 'text-red-400' : 'text-gray-400'}`}>{caption.length}/150</p>
              </div>
            </div>
